        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[1000] p-4">
            <div className="bg-white rounded-lg max-w-md w-full p-6 shadow-2xl">
                ...
                <div className="flex justify-end space-x-3">
                    <button onClick={onClose} className="px-4 py-2 text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50" disabled={isProcessing}>{cancelText}</button>
...
export default function AccountLayout() {
    ...
    const activeTab = useMemo(() => {
        ...
        if (path.length === 1 || path[1] === 'profile') return 'profile';
    ...
    if (userData.role === "owner") {
        menuItems.push({ id: "admin", label: "Panel administratora", icon: Shield });
    }
        ...
        <div className="min-h-screen bg-gray-50 py-8">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="mb-8 pt-8">
                    ...
                    <p className="text-gray-600 mt-2">Witaj, {userData.firstName}! Zarządzaj swoim kontem.</p>
                </div>
                ...
                {successMessage && (
                    <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded mb-6 flex items-center justify-between">
                        <span className="flex items-center gap-2"><CheckCircle className="h-5 w-5" />{successMessage}</span>
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                                    ...
                                    {menuItems.map((item) => (
                                        <li key={item.id}>
                                            <Link
                                                ...
                                                className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors ${activeTab === item.id ? 'bg-black text-white' : 'text-gray-700 hover:bg-gray-100'}`}
                                            >
                            ...
                            <div className="p-4 border-t border-gray-200">
                                <button onClick={() => setShowLogoutModal(true)} className="w-full flex items-center justify-center space-x-2 text-red-600 hover:bg-red-50 py-2 rounded-lg"><LogOut className="h-5 w-5" /><span>Wyloguj się</span></button>
                            </div>
                        </div>
                    </div>
                ...
                <ConfirmationModal show={showLogoutModal} onClose={() => setShowLogoutModal(false)} onConfirm={confirmLogout} title="Wylogowanie" message="Czy na pewno chcesz się wylogować?" confirmText="Wyloguj się" type="danger" isProcessing={isLoggingOut} />
            </div>
        </div>
    );
}