...
import { ShoppingCart, Trash2, ArrowRight, Minus, Plus, XCircle, LogIn, UserPlus } from "lucide-react";
...
export default function Cart() {
    ...
    const handleCheckout = () => {
        // Nawigacja do Checkout
        navigate("/checkout");
    };

    // Widok dla pustego koszyka
    if (count === 0) {
        return (
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    {cartItems.map((item) => (
                        <div key={item.productId} className="flex items-center bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                            ...
                            <div className="flex-1 min-w-0 mx-4">
                                ...
                                <p className="text-sm text-gray-500">Cena jednostkowa: {formatPrice(item.price)}</p>
                            </div>
                            ...
                            <div className="flex items-center space-x-2 border border-gray-300 rounded-lg p-1">
                                <button
                                    ...
                                    className="text-gray-600 hover:bg-gray-100 p-1 rounded disabled:opacity-50"
                ...
                {/* Prawa kolumna: Podsumowanie i przycisk Checkout */}
                <div className="lg:col-span-1">
                    <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200 sticky top-24">
                        ...
                        {/* Warunkowe wyświetlanie przycisku Checkout/Logowania */}
                        {isAuthenticated ? (
                            <button
                                onClick={handleCheckout}
                                className="w-full mt-6 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2"
                            >
                                <span>Przejdź do kasy</span>
                                <ArrowRight className="h-5 w-5" />
                            </button>
                        ) : (
                            <div className="mt-6 p-4 border border-yellow-300 bg-yellow-50 rounded-lg space-y-3">
                                <p className="text-sm font-medium text-yellow-800">
                                    Musisz się zalogować, aby kontynuować zamówienie.
                                </p>
                                <div className="flex space-x-3">
                                    <Link to="/login" className="flex-1 py-2 bg-black text-white text-center rounded-lg text-sm hover:bg-gray-800 flex items-center justify-center space-x-1">
                                        <LogIn className="h-4 w-4" /> <span>Zaloguj się</span>
                                    </Link>
                                    <Link to="/register" className="flex-1 py-2 border border-gray-300 text-gray-700 text-center rounded-lg text-sm hover:bg-gray-100 flex items-center justify-center space-x-1">
                                        <UserPlus className="h-4 w-4" /> <span>Zarejestruj się</span>
                                    </Link>
                                </div>
                            </div>
                        )}
